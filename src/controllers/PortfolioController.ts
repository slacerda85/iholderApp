...
class PortfolioController {
  async create(request: Request, response: Response, next: NextFunction) {
    try {
      const { asset_ticker } = request.body
  ...
  async show(request: Request, response: Response, next: NextFunction) {
    try {
      ...
      const asset: Portfolio[] = await knex('portfolio').where({
        asset_ticker: asset_ticker.toUpperCase()
      })
  ...
  async update(request: Request, response: Response, next: NextFunction) {
    try {
      const { qtd, avg_price } = request.body
      const { asset_ticker } = request.params

      await knex('portfolio')
        .update({
          qtd: qtd,
          avg_price: avg_price,
